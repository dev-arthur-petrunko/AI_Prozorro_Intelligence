...
- **🔄 Автоматична синхронізація** — кожні 30 хвилин підтягує нові тендери з відкритого API Prozorro (без дублікатів: унікальний індекс, перевірка перед імпортом і щоденна страхувальна дедуплікація)
...
- **📅 Щоденні звіти** — зведення дня + автоматична відправка в Telegram через n8n webhook (щоденний звіт о 13:00 та миттєві сповіщення про активні тендери з високим індексом у вікні 9:00–20:00)
...
| Переможець регулярно виграє у замовника | +20 | 3+ попередніх перемог у того ж замовника |
| Ротація сталого пулу учасників | +15 | 2–3 компанії ділять 4+ перемог у одного замовника |
...
**3. Пороги:** 0–30 низький · 31–60 середній · 61–80 високий · 81+ критичний. Тендери з індексом > 30 отримують аналітичний коментар (AI) за 9 критеріями. У топах дашборда сортування йде за **пріоритетом уваги** = індекс × log₁₀(сума), щоб дрібні закупівлі не витісняли великі.

**4. Життєвий цикл коментаря:** генерується один раз; переаналіз — лише якщо у незавершеного тендера зʼявились нові дані. Завершені тендери з готовим коментарем не переаналізовуються.
